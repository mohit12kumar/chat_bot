...
def main():
    ...
    print("😠 Starting Rude FastAPI Backend...")
    # Run FastAPI
    backend_process = subprocess.Popen([sys.executable, backend_script])

    print("😠 Starting Rude Streamlit Frontend...")
    # Run Streamlit on port 8502
    frontend_process = subprocess.Popen([sys.executable, "-m", "streamlit", "run", frontend_script, "--server.port", "8502"])
